...
'''Object-oriented graphics library that modifies the
   ...
   Latest version modified February/March 2020 by JS Iwanski
   to add:
...
class DblEntry(Entry):
    ...
    def checkValue(self):
        '''returns true if valid entry, otherwise return false'''
        text = self.getText()
        if not _is_number(text):
            self.setFill(self.errorcolors[0])
            self.setTextColor(self.errorcolors[1])
...
#### A simple static button class ####
class SimpleButton:

    '''A simple button is a labeled rectangle in a window.
    ...
    various properties, including:
        ...
        > the width and color of the button's edge
    ...
    def __init__(self, win, topLeft, width, height,
                 ...
                 label = 'button label', font=('courier',18),
                 buttonColor = 'lightgray', textColor = 'black'):
    ...
    def setEdgeColor(self, newColor):
        '''Sets the edge color to newColor'''
        self.edgecolorNew = newColor
        self.rect.setOutline(self.edgecolorNew)
...
class DropDown(GraphicsObject):
    def __init__(self, topLeft,choices=[], width=10, font=('courier',18),bg='black'):
        GraphicsObject.__init__(self, [])
